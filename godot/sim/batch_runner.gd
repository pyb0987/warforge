extends SceneTree
## Autoresearch batch runner. Entry point for headless simulation.
##
## Usage:
##   godot --headless --path godot/ -s sim/batch_runner.gd -- \
##     --genome=res://sim/default_genome.json --runs=10 --seed=42
##
## Output: JSON evaluation result to stdout.


func _init() -> void:
	# Defer all work to after Autoloads are registered.
	# In -s mode, _init() runs before SceneTree adds Autoload nodes to /root.
	# process_frame fires after the first frame, when Autoloads are available.
	process_frame.connect(_run_batch, CONNECT_ONE_SHOT)


func _run_batch() -> void:
	# Runtime-load to avoid compile-time Autoload resolution with -s flag.
	...
	var runs_per_ai: int = args.get("runs", 10)
	...
	printerr("=== Batch Runner ===")
	printerr("Genome: %s" % genome_path)
	printerr("Runs per AI: %d × %d strategies = %d total" % [runs_per_ai, strategies.size(), total])
	...
	printerr("Evaluating %d results..." % results.size())
	var score: Dictionary = _EvalClass.evaluate(results)
	...
	# Compute axis delta if baseline provided
	...
	printerr("=== Done ===")
	printerr("Weighted score: %.4f" % score.weighted_score)
	quit(0)
